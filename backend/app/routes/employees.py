...
def create_employee(body: EmployeeCreate, db: Session = Depends(get_db), _=Depends(validate_token)):
...
def update_employee(emp_id: str, body: EmployeeUpdate, db: Session = Depends(get_db), _=Depends(validate_token)):
